...
#define foreach_osvbng_punt_pppoe_sess_error \
  _(PUNTED, "PPPoE Session packets punted")  \
  _(DROPPED, "PPPoE Session packets dropped (socket error)")
...
format_osvbng_punt_pppoe_sess_trace (u8 * s, va_list * args)
{
  ...
  s = format (s, "OSVBNG-PUNT-PPPOE-SESS: sw_if_index %d %s",
	      t->sw_if_index, t->punted ? "punted" : "dropped");
  return s;
}
			       ...
			       vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  /* PPPoE packets are at ethernet-input, buffer already points to Ethernet header */
	  /* No need to rewind - we're already at the right position */

	  /* Send packet to userspace */
	  if (osvbng_punt_send_packet
	      (b0, sw_if_index0, OSVBNG_PUNT_PROTO_PPPOE_SESSION) == 0)
	    {
	      pkts_punted++;
	      punted = 1;
	    }
...
VNET_FEATURE_INIT (osvbng_punt_pppoe_sess, static) = {
  ...
  .runs_before = VNET_FEATURES ("ethernet-input-not-l2"),
};
